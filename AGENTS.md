...
- **Browser (opt-in module):** Steel browser **Docker container** on `:3877`
  (`STEEL_PORT`; deliberately off `:3000` so user dev servers can use it —
...
Internet → Caddy (TLS) → FastAPI :8000
                           ...
                           │         server (owner-token gated; dotfiles denied; only
                           │         explicitly published paths are open — ~/public is
                           │         NOT ambient, publishing is the sole grant)
