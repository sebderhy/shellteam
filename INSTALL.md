...
> Ports taken? They're all configurable in `.env` (`API_PORT`, `AI_CHAT_PORT`,
> `FILE_PORT`, and `STEEL_PORT` for the browser module) — see [§6 Changing
> ports](#changing-ports). The installer also auto-picks the next free port if a
> default is busy. You don't need the
...
`:3877` (`STEEL_PORT` — deliberately off `:3000` so your own dev servers can
claim that port; bundles its own pinned Chromium). This is the one piece that needs
...
# Dashboard renders the tab shell (Agents/Terminal/Files/Browser/Apps/Settings,
# plus a hidden Knowledge tab that appears only with the dreaming module):
curl -fsS -H "Host: localhost" http://127.0.0.1:8000/ | grep -c '<button class="tab'   # → 8
...
curl -fsS "https://${DOMAIN}/" -H "Authorization: Bearer ${TOKEN}" | grep -c '<button class="tab'   # → 8
...
### 5.1 Mirror your laptop (optional, `--full` installs)

Coming from a Mac, Linux, or WSL setup you've spent years tuning? The dashboard
generates a one-command import for it: open **Settings → Import your laptop**
(it's also the last step of the first-run wizard), copy the command, and run it
on your **laptop**. It first asks which folder to scan — your home by default;
on WSL it also offers your Windows profile (`C:\Users\<you>`), where the real
setup usually lives (`MIRROR_SCAN_HOME=<dir>` skips the question). It then opens
a local wizard in your browser where you tick what moves in — packages,
dotfiles, repos, agent setup, agent logins and authenticated MCPs (skip
re-login on the box), Claude/Codex chat history, shell history — then uploads
your selection over TLS straight to this box, which starts the migration by
itself.

The privacy model has two tiers:

- The **default tarball** ("Just save a file", or running
  `mirror-inventory.sh` by hand) never contains secrets: credential files are
  excluded by construction, secret-looking lines are redacted, and secret
  *locations* are mapped so the agent walks you through re-provisioning them
  fresh. Safe to store or move by hand.
- **Direct upload only**: agent logins, MCP tokens, and chat history are
  opt-in checkboxes that travel exclusively over TLS to your own box (the
  command embeds a 24-hour upload pass, not your master token) and are never
  written into a saved file. SSH private keys never travel either way — the
  box generates fresh ones.

Everything the command fetches comes **from your own box** (it serves the
wizard scripts at its own installed version), so no third party — and no moved
git ref — ever sits in the path of code that reads your laptop.

Manual fallback (no wizard, secret-free tarball) — download the script, read
it (it is strictly read-only on your machine), then run it:

```bash
curl -fsSLO https://your-box.example.com/api/mirror/mirror-inventory.sh
less mirror-inventory.sh    # whitelist-only inventory; see for yourself
bash mirror-inventory.sh
```

Review it (`tar -tzf`), upload it to the box (Files tab or `scp`), and tell
your agent: *"mirror my laptop from the shellteam-mirror tarball"*. Either way
the `migrate` skill translates with judgment (brew→apt, mac-isms adapted, GUI
apps skipped with a note, laptop paths remapped in chat history) and finishes
with a migration report of what mapped and what didn't.
...
The service ports all live in `.env` — there is **one** place to change each,
...
| Steel browser (browser module only) | `STEEL_PORT` | `3877` | Loopback container port. Deliberately off `:3000` so your own dev servers can use it; the installer auto-picks a free port on collision. |

All four are reserved from your own apps: a request to `owner-<that-port>` is never proxied to a user server, and none can be made public.
...
or run `./install.sh --plan` (with the flags you intend to use) to print it for
your exact setup before installing.
