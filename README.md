...
| **Apps over MCP** | Bring your own MCP servers, or flip on Composio with one key for 500+ app connections (the dashboard's Apps tab connects them). Off by default. |
...
└── docker container (opt-in; part of --full)
      └── shellteam-steel  :3877  ← Steel browser + bundled Chromium (STEEL_PORT, off :3000 for your apps)
```
...
[docs/FOOTPRINT.md](docs/FOOTPRINT.md) — and `./install.sh --plan` prints that
manifest for your exact flags, evaluated against your box, before you commit to
anything.
...
that makes that setup usable away from your desk. The agents get no new
privilege: they already had your box. What changes is that they run in
full-auto mode (no approval prompts) and the box gets a token-gated URL.
[SECURITY.md](SECURITY.md) spells out exactly that difference and how to
confine agents to their own user on a shared machine.
