...
# ShellTeam's own service ports. API_PORT/AI_CHAT_PORT mirror what install.sh
# renders into the systemd units; STEEL_PORT is the host-side loopback port the
# Steel browser container is published on. Steel deliberately defaults OFF
# :3000 — that is the default dev port of Next.js/CRA/Rails, and a user's app
# must be able to claim it (the container previously hardcoded 3000 and any
# `npm run dev` on a --full box died with EADDRINUSE).
API_PORT = int(os.environ.get("API_PORT", "8000"))
AI_CHAT_PORT_NUM = int(os.environ.get("AI_CHAT_PORT", "3456"))
STEEL_PORT = int(os.environ.get("STEEL_PORT", "3877"))


def reserved_service_ports() -> set[int]:
    """Ports owned by ShellTeam's own services — never publishable, never
    shareable, and (for the control plane itself) never proxied. Keeping this a
