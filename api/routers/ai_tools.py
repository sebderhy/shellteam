...
from api.services import feature_keys, stt, tts
...
async def _verify_stt_token(
    request: Request,
    creds: HTTPAuthorizationCredentials = Depends(_bearer),
) -> None:
    """Auth for the voice routes (/stt, /status): the in-box token, or — org
    module — a per-guest scoped STT capability token.

    Employee cockpits deliberately never hold the master ``SHELLTEAM_AI_TOKEN``
    (it unlocks every /internal/ai capability on the owner's keys, including the
    Fireworks proxy). Voice input is the one AI capability they DO get, so it
    rides its own token, per the one-token-one-capability rule. The Fireworks
    routes keep ``_verify_token`` and never accept it.
    """
    user_id = request.headers.get("X-Shellteam-User-Id", "").strip()
    if user_id and verify_token(creds.credentials, user_id):
        request.state.rate_limit_user_id = user_id
        return
    raise HTTPException(status_code=401, detail="Invalid token")
...
@router.get("/status", dependencies=[Depends(_verify_stt_token)])
async def ai_status(request: Request) -> dict:
    ...
    A guest STT token sees only the ``stt`` flag: its holder's single
    capability. Everything else reads false in their cockpit, which is also
    the truth — the other capabilities are not lent to them.
    """
    status = feature_keys.capability_status()
    if getattr(request.state, "stt_guest", ""):
        return {"stt": status.get("stt", False)}
    return status
...
@router.post("/stt", dependencies=[Depends(_verify_stt_token), Depends(_ai_limit)])
...
# --- Text-to-speech (cockpit spoken replies) ---
#
# Its own budget rather than _ai_limit's 50/day: voice INPUT is something you
# do a few times a day, voice OUTPUT happens once per assistant turn, so
# sharing a counter would silence replies mid-afternoon. The real spend
# backstop is the character cap in tts.py plus the ElevenLabs plan itself.
_tts_limit = RateLimiter(rate=300, period=86400, key="user")
...
@router.post("/tts", dependencies=[Depends(_verify_token), Depends(_tts_limit)])
async def text_to_speech(req: SpeakRequest):
    """Speak text with the owner's ElevenLabs key. Returns raw MP3 bytes.

    Deliberately behind ``_verify_token``, not ``_verify_stt_token``: a guest's
    scoped voice token buys exactly one capability (transcription), and spending
    the owner's synthesis quota is a different one. Granting voice output to
    employees means adding a token for it, never widening this one.
    """
    try:
...
# --- Fireworks proxy (the OpenCode family) ---
#
