...
# Where Composio sends the browser back after OAuth: the Apps tab shell, which
# syncs CLI credentials, refreshes the grid, and closes itself when it runs in
# the popup the tab opened (frontend/apps.html).
APP_CONNECTED_PATH = "/apps"
...
async def search_toolkits(
    ...
    """Search Composio's app catalog for the Apps tab's "find any app" box."""
    _require_composio_configured()
    query = q.strip()
    if len(query) < 2:
        raise HTTPException(status_code=400, detail="Type at least 2 characters")
    try:
        results = await asyncio.to_thread(composio_svc.search_toolkits, query)
    except Exception as e:
        log.warning("Composio toolkit search %r failed: %s", query, e)
        raise HTTPException(status_code=502, detail=f"Composio catalog search failed: {e}")
    return [Toolkit(**t) for t in results]


@router.get("/logo/{toolkit}")
async def toolkit_logo(
    toolkit: str,
    user: dict = Depends(get_current_user),
):
    """A toolkit's logo, relayed from Composio's CDN (see composio.fetch_logo)."""
    _require_composio_configured()
    if not TOOLKIT_RE.match(toolkit):
        raise HTTPException(status_code=400, detail="Invalid toolkit slug")
    try:
        content_type, data = await asyncio.to_thread(composio_svc.fetch_logo, toolkit)
    except Exception as e:
        log.info("No Composio logo for %r: %s", toolkit, e)
...
async def connect_app(
    ...
    try:
        redirect_url = await asyncio.to_thread(
            ...
            user["id"],
            toolkit,
            f"{APP_URL}{APP_CONNECTED_PATH}?app_connected={toolkit}",
            config_params,
        )
    except Exception as e:
