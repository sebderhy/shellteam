...
from api.config import APP_DOMAIN, APP_URL, OWNER_USERNAME, RUNTIME
...
async def mint_port_share_internal(body: InternalPortShareRequest, request: Request):
    ...
    if body.port == FILE_PORT or body.port in reserved_service_ports():
        raise HTTPException(
            status_code=400,
            detail=f"Port {body.port} belongs to a ShellTeam service and cannot be shared",
        )
    exp = int(time.time()) + body.ttl
    sig = sign_share_port(body.port, exp)
    log.info("Minted port-share link for :%d via /internal (ttl=%ss)", body.port, body.ttl)
    return {
        "url": f"https://{OWNER_USERNAME}-{body.port}.{APP_DOMAIN}/?sig={sig}&exp={exp}",
        "expires_at": exp,
        "expires_in": body.ttl,
    }
