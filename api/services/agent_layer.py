...
from api.config import APP_DOMAIN, MODULES, STEEL_PORT
...
# Loopback CDP endpoint of the Steel browser container (see install.sh). The
# host-side port is STEEL_PORT (default 3877 — deliberately off :3000, which
# belongs to the user's own dev servers); the template placeholder stays the
# literal `ws://127.0.0.1:3000` and is rewritten here.
BROWSER_CDP_ENDPOINT = os.environ.get(
    "BROWSER_CDP_ENDPOINT", f"ws://127.0.0.1:{STEEL_PORT}"
)
...
def _skill_required_bins(skill_md_text: str) -> list[str]:
    """Binaries a skill declares as hard requirements in its SKILL.md
    frontmatter (any ``requires: {bins: [...]}`` nested under ``metadata``, the
    convention the ``gws-*`` skills ship as ``metadata.openclaw.requires.bins``).

    No CLI enforces this at runtime. Verified empirically against Claude Code
    v2.1.215: the string ``openclaw`` does not appear in the bundle and no
    ``bins`` occurrence relates to skill gating, so the frontmatter is inert
    metadata to every agent. The gate therefore lives at layer-build time
    (``_copy_rendered_skills``)."""
    ...
    def collect(node: object) -> None:
        ...
        if isinstance(requires, dict) and isinstance(requires.get("bins"), list):
            bins.extend(str(b) for b in requires["bins"])
        for value in node.values():
            collect(value)

    collect(frontmatter.get("metadata") or {})
    return bins
...
def _copy_rendered_skills(
    ...
    Skills whose frontmatter declares required binaries that are not on PATH
    are skipped: shipping them would teach the agent commands that cannot run
    (the ``gws-*`` family needs the ``gws`` CLI, which native installs do not
    provision). The layer is rebuilt on every API boot, so installing the
    binary makes the skills appear on the next restart."""
    ...
    for skill_dir in sorted(skills_src.iterdir()):
        ...
        missing = [b for b in _skill_required_bins(skill_text) if shutil.which(b) is None]
        if missing:
            log.info(
                "Skipping skill '%s': required binaries not on PATH: %s "
                "(declared in SKILL.md frontmatter requires.bins; install them "
                "and restart to enable the skill)",
                skill_dir.name, ", ".join(missing),
            )
            continue
        dst = destination / skill_dir.name
        shutil.copytree(skill_dir, dst)
        (dst / "SKILL.md").write_text(render(skill_text))
