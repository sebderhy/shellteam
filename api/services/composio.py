...
LOGO_BASE = "https://logos.composio.dev/"
# The Apps tab shows logos through the control plane (/api/integrations/logo/
# <slug>) rather than straight from Composio's CDN: the dashboard's cross-origin
# isolation blocks foreign images, and the owner's browser should not fan out to
# a third party to draw a grid. Cached for the process lifetime; logos are tiny.
LOGO_PATH = "/api/integrations/logo/"
_logo_cache: dict[str, tuple[str, bytes]] = {}


def _shape_toolkit(item: dict) -> dict:
    """The subset of a Composio toolkit record the Apps tab renders.

    ``managed`` is True when Composio hosts the OAuth app for this toolkit, so
    the owner can connect it with a click and no developer setup. Toolkits
    that only take an API key or a self-registered OAuth app are still listed,
    but the UI hands those to the agent (Tool Router's manage-connections flow
    walks the user through the key) instead of a dead Connect button.
    """
    ...
    return {
        ...
        "logo": f"{LOGO_PATH}{item['slug']}" if logo.startswith(LOGO_BASE) else "",
        "managed": bool(
            {"OAUTH2", "OAUTH1"} & set(item.get("composio_managed_auth_schemes") or [])
        ),
    }


def search_toolkits(query: str, limit: int = 12) -> list[dict]:
    """Search Composio's toolkit catalog by name (REST, same key as the SDK)."""
    ...
    log.info("Composio toolkit search %r returned %d results", query, len(items))
    return [_shape_toolkit(i) for i in items]


def fetch_logo(slug: str) -> tuple[str, bytes]:
    """(content-type, bytes) of a toolkit's logo from Composio's CDN, cached."""
    if slug in _logo_cache:
        return _logo_cache[slug]
    resp = httpx.get(f"{LOGO_BASE}api/{slug}", timeout=10, follow_redirects=True)
    resp.raise_for_status()
    content_type = resp.headers.get("content-type", "")
    if not content_type.startswith("image/"):
        raise ValueError(f"Composio returned {content_type or 'no content-type'} for logo {slug!r}")
    _logo_cache[slug] = (content_type, resp.content)
    return _logo_cache[slug]
...
def get_required_fields(toolkit: str) -> list[dict]:
    ...
    api_key = os.environ.get("COMPOSIO_API_KEY", "")
    resp = httpx.get(
        f"{COMPOSIO_API_BASE}/toolkits/{toolkit}",
        headers={"x-api-key": api_key},
    )
    resp.raise_for_status()
    data = resp.json()
