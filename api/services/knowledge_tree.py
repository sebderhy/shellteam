...
def apply_delta(
    ...
    facts wait in the OPTIONAL review queue (``DREAM_REVIEW=all`` queues
    everything instead). ``force=True`` (owner approval) applies a queued fact,
    never bypassing scope/injection.
...
def _apply_one(
    ...
    # DREAM_REVIEW=all flips that trade: EVERY proposal queues for owner
    # review before it can influence future agents — the paranoid mode for
    # owners who treat consolidated memory as an injection-persistence surface
    # (SECURITY.md "Dreaming and memory poisoning").
    if not force:
        if (os.environ.get("DREAM_REVIEW") or "low") == "all":
            _queue_for_review(home, run, node, op)
            return "queued", "review=all"
        if op.get("confidence") == "low":
            _queue_for_review(home, run, node, op)
