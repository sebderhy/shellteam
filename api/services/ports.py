...
def is_port_public(user_id: str, port: int) -> bool:
    # Enforce reservation at the READ boundary, not just the setter: a box
    # provisioned before reservation existed could have :3456 (cockpit) or an
    # old Steel :3000 already in public_ports.json, and trusting that stored
...
def set_port_visibility(user_id: str, port: int, public: bool) -> set[int]:
    ...
    # ShellTeam's own services must never be flipped world-readable: a public
    # API_PORT would put the control plane behind `owner-8000.APP_DOMAIN` with
    # the auth gate skipped entirely (is_public short-circuits _authorize).
    if public and port in reserved_service_ports():
        raise ValueError(
            f"Port {port} belongs to a ShellTeam service and cannot be made public"
        )
...
def seed_from_disk() -> int:
    ...
    for user_dir in DATA_DIR.iterdir():
        ...
        try:
            ...
            # Scrub any reserved service port a pre-reservation box may have
            # persisted, and rewrite the file so the bad grant is gone for good.
            reserved = ports & reserved_service_ports()
            if reserved:
                log.warning(
                    "Dropping reserved service port(s) %s from public grant for %s "
                    "(pre-reservation state)", sorted(reserved), user_dir.name,
                )
                ports -= reserved
                _persist(user_dir.name, ports)
            if ports:
