...
# Dedicated limiter for file reads that MISS the publish allowlist (a 401). The
# global 120/min net treats a legitimate reader of a published page (all hits)
# and a slug-guessing enumerator (all misses) identically; a reader essentially
# never misses, so a tight miss budget throttles enumeration without touching
# real use (exposure-audit F6). 20 misses / minute / IP, then 429.
file_miss_limiter = RateLimiter(rate=20, period=60)


def note_unpublished_read(request: Request) -> None:
    """Record a read that hit a non-published path; raise 429 once an IP has
    exceeded the miss budget. Detection matters as much as blocking — sustained
    misses are logged so the exposure panel can surface them."""
    if not file_miss_limiter.allow(request):
        ip = request.client.host if request.client else "unknown"
        log.warning("Enumeration backoff tripped for %s — too many unpublished-path reads", ip)
        raise HTTPException(status_code=429, detail="Too many requests")
