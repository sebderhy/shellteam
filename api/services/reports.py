"""Per-path public visibility for the owner's shareable files.

A "report" is a file under the owner's home (by convention ``~/reports/…`` or
``~/public/…``). By default it is owner-only — served only to the authenticated
owner, exactly like the rest of ``$HOME``. Publishing adds its home-relative
path to this allowlist; the proxy then serves that path without cookie auth, at
the **same URL** (no file move, no copy). Un-publish removes it. This mirrors
the private→public model in ``ports.py``.

An entry may be a DIRECTORY (e.g. ``public/logos``): everything under it is
served — one publish action for a static bundle with relative links. This is
the ONLY publish mechanism on the box: ``~/public`` is a storage convention,
not an ambient grant — writing a file there does NOT put it on the internet
(that only took one stray ``cp`` or one prompt-injected agent write; see
...
def resolve_report_path(home_dir: Path, path: str, owner_initiated: bool = False) -> str:
    ...
    ``owner_initiated`` lifts the reports/ + public/ confinement: the owner
    clicking Publish in their own editor may publish any file in their home,
    which is the whole point of the button. The confinement stays for every
    machine caller (``/internal/reports``), because that token is held by every
    in-box agent — including a prompt-injected one — and the subtree list is
    what stops it turning ``~/backup.sql`` into a public URL (M3). Dotfiles and
    anything outside home are refused for BOTH callers.
    """
    ...
    # Dotfiles first: it is the rule that binds every caller, so a dotfile must
    # say so rather than hiding behind the (caller-dependent) subtree message.
    if any(seg.startswith(".") for seg in Path(rel).parts):
        raise ValueError("Dotfiles cannot be published")
    if not owner_initiated and not _is_publishable_subtree(rel):
        raise ValueError("Only files under reports/ or public/ can be published")
    # Directories are publishable too (one action for a static bundle with
    # relative links) — everything under the entry becomes world-readable.
...
def is_report_public(user_id: str, relpath: str) -> bool:
    ...
    if rel in entries:
        return True
    return any(rel.startswith(entry + "/") for entry in entries)
...
_MIGRATION_MARKER = "public_grant_migrated"


def migrate_ambient_public_grant(user_id: str, home_dir: Path) -> list[str]:
    """One-time upgrade shim for removing the ambient ``~/public`` mount.

    Boxes upgraded from ≤0.1.6 have live ``/public/…`` links that used to be
    served with no publish entry. Auto-publish each TOP-LEVEL item already in
    ``~/public`` (dirs as directory entries) so no existing link breaks, and
    say exactly what is world-readable at WARNING — for most owners this is
    the first actual inventory of what they were hosting. Runs once (marker
    file); a fresh box marks itself migrated with zero entries.
    """
    marker = DATA_DIR / user_id / _MIGRATION_MARKER
    if marker.exists():
        return []
    ...
    if public_dir.is_dir():
        ...
        for entry in sorted(public_dir.iterdir()):
            if entry.name.startswith("."):
                continue
            rel = f"public/{entry.name}"
            if not is_report_public(user_id, rel):
                current = current | {rel}
                migrated.append(rel)
        if migrated:
            # Deliberately bypasses MAX_PUBLIC_REPORTS: these were ALREADY
            # world-readable — recording that fact must not fail.
            _public_reports[user_id] = current
            _persist(user_id, current)
    marker.parent.mkdir(parents=True, exist_ok=True)
    marker.write_text("~/public ambient grant removed; existing content auto-published\n")
    if migrated:
        log.warning(
            "~/public is no longer served automatically. %d existing item(s) were "
            "auto-published so links keep working — this is what is world-readable: %s. "
            "Review and unpublish in the dashboard's Settings → Exposure panel.",
            len(migrated), ", ".join(migrated),
        )
    else:
        log.info("~/public ambient-grant migration: nothing to publish (empty folder).")
    return migrated


def exposure(user_id: str, home_dir: Path) -> list[dict]:
    """Every world-readable path, with per-entry file counts — the data behind
    ``GET /api/exposure``, the boot log line, and the dashboard panel."""
    entries = []
    for rel in sorted(get_public_reports(user_id)):
        abs_path = home_dir / rel
        if abs_path.is_dir():
            files = sum(1 for f in abs_path.rglob("*") if f.is_file())
            entries.append({"path": rel, "kind": "directory", "files": files})
        else:
            entries.append({"path": rel, "kind": "file", "exists": abs_path.is_file()})
    return entries
