...
function envFallback() {
  return {
    ...
    tts: !!process.env.ELEVENLABS_API_KEY && !!process.env.SHELLTEAM_AI_TOKEN,
  };
}

/**
 * The current availability flags `{ opencode, stt, tts }` — the cached
...
export async function refreshAiAvailability({ fetchImpl = fetch } = {}) {
  ...
  try {
    ...
    _cached = { opencode: !!data.opencode, stt: !!data.stt, tts: !!data.tts };
    return _cached;
  } catch (err) {
    console.warn(
