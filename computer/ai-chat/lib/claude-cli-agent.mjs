...
export class ClaudeCliAgent extends CodingAgent {
  constructor(opts) {
    ...
    // A pending AskUserQuestion permission request (control protocol): the CLI
    // is blocked waiting for our control_response until the user picks an
    // option (or types a normal message, which denies it).
  ...
  sendMessage(content) {
    ...
    // Typing a normal message while a question waits for its buttons means the
    // user answered in prose — release the blocked tool call so the CLI reads
    // this message instead of hanging on the control_response forever.
    this._cancelPendingQuestion("The user replied in the chat instead of selecting an option. Read their next message for the answer.");
  ...
  /**
   * Answer a pending AskUserQuestion. `answers` is the record the tool's
   * schema expects: question text -> chosen label (array when multiSelect).
   * Returns false when the request is stale (turn ended, process died) so the
   * caller can fall back to delivering the choice as a normal message.
   */
  answerQuestion(requestId, answers) {
    const pending = this._pendingQuestion;
    if (!pending || pending.requestId !== requestId || !this._process) {
      console.warn(`[claude-cli] answerQuestion for ${requestId} refused — pending=${pending?.requestId || "none"} process=${!!this._process}`);
      return false;
    }
    this._pendingQuestion = null;
    console.log(`[claude-cli] Answering question ${requestId}: ${JSON.stringify(answers).slice(0, 200)}`);
    this._writeControlResponse(requestId, {
      subtype: "success",
      response: { behavior: "allow", updatedInput: { questions: pending.questions, answers } },
    });
    this._resetWatchdog();
    return true;
  }

  _cancelPendingQuestion(reason) {
    const pending = this._pendingQuestion;
    if (!pending) return;
    this._pendingQuestion = null;
    if (this._process) {
      console.log(`[claude-cli] Cancelling pending question ${pending.requestId}: ${reason}`);
      this._writeControlResponse(pending.requestId, {
        subtype: "success",
  ...
  _writeControlResponse(requestId, response) {
    const line = JSON.stringify({ type: "control_response", response: { request_id: requestId, ...response } }) + "\n";
    this._process?.stdin.write(line);
  }

  interrupt() {
    // SIGINT aborts the whole turn — the blocked tool call dies with it.
    this._pendingQuestion = null;
    if (this._process) {
  ...
  _spawnCLI() {
    const args = [
      ...
      // Permission checks stay bypassed, but registering a stdio permission
      // handler is what puts AskUserQuestion in the headless tool list: the CLI
      // routes requires_user_interaction tools here as can_use_tool
      // control_requests, which we surface as clickable choices in the cockpit.
  ...
  _handleMessage(msg) {
    ...
    // --- control_request (permission protocol; AskUserQuestion arrives here) ---
    ...
    if (msg.type === "assistant" && !isSubagent) {
      ...
      for (const block of content) {
        if (block.type === "text" && block.text && !this._emittedBlockIds?.has(block.id)) {
          ...
          // AskUserQuestion is NOT emitted from the message stream: the
          // can_use_tool control_request is the single source, because only it
          // carries the request_id the answer must reference.
    ...
    if (msg.type === "user" && msg.message?.content && !isSubagent) {
      ...
      for (const block of content) {
        this._trackBackgroundTask(block);
  ...
  _handleControlRequest(msg) {
    ...
    if (req.subtype !== "can_use_tool") {
      // Nothing else is expected on this channel — answer with an error so the
      // CLI never blocks on a request we don't understand.
      console.warn(`[claude-cli] Unsupported control_request subtype=${req.subtype} — refusing`);
      this._writeControlResponse(msg.request_id, {
        subtype: "error",
        error: `unsupported control_request subtype: ${req.subtype}`,
      });
      return;
    }
    if (req.tool_name === "AskUserQuestion") {
      ...
      // The turn is now waiting on a human, not the CLI — the watchdog must
      // not read the silence as a wedge. Re-armed when the answer goes back.
      this._clearWatchdog();
      console.log(`[claude-cli] AskUserQuestion pending (${this._pendingQuestion.questions.length} question(s), request ${msg.request_id})`);
      this.emit("ask_user", {
        id: req.tool_use_id,
        requestId: msg.request_id,
        questions: this._pendingQuestion.questions,
      });
      return;
    }
    // --dangerously-skip-permissions bypasses every ordinary permission check,
    // so only other requires_user_interaction tools land here. Auto-allow
    // preserves the pre-control-protocol behavior (the tool just runs).
    console.log(`[claude-cli] Auto-allowing control_request for ${req.tool_name}`);
    this._writeControlResponse(msg.request_id, {
      subtype: "success",
  ...
  _finalizeToolInput(toolId) {
    ...
    if (name === "AskUserQuestion") {
      // Emitted from the control_request instead (it carries the request_id).
      return;
    }
  ...
  // --- Background Bash tracking (idle-reap deferral) ---
