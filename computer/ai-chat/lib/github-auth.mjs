...
// ─── Repo picker: list the user's repos, clone the selected ones ─────────────
// Clones land straight in ~/<repo>: the workspace picker lists every top-level
// folder under $HOME (workspaces.mjs), so a repo here is immediately
// selectable, and that is how boxes are actually laid out in practice.

const CLONE_ROOT = homedir();
const REPO_NAME_RE = /^[\w.-]+\/[\w.-]+$/;

function cloneDest(nameWithOwner) {
  return join(CLONE_ROOT, nameWithOwner.split("/")[1]);
}

// What is sitting at a repo's destination folder already. Cloning into $HOME
// means a repo name can collide with an unrelated folder (a ~/notes that has
// nothing to do with the GitHub repo of that name), so "already here" and
// "name taken" are reported apart — calling a stranger's folder the repo
// would be a lie.
function destState(nameWithOwner) {
  const dest = cloneDest(nameWithOwner);
  if (!existsSync(dest)) return "free";
  return existsSync(join(dest, ".git")) ? "cloned" : "occupied";
}

/**
 * The signed-in user's repositories (most recently pushed first), each with
 * `cloned` / `occupied` describing its destination in $HOME. Errors propagate
 * to the route.
 */
export async function listRepos() {
  ...
  return JSON.parse(stdout).map((repo) => {
    const state = destState(repo.nameWithOwner);
    return { ...repo, cloned: state === "cloned", occupied: state === "occupied" };
  });
}

// nameWithOwner → { status: "cloning" | "done" | "failed", error? }. In-memory
// only: jobs are seconds-to-minutes and the UI re-derives "already on the box"
// from listRepos() after a restart.
const cloneJobs = new Map();
let cloneQueue = Promise.resolve();

export function cloneStatus() {
  return Object.fromEntries(cloneJobs);
}

/**
 * Queue clones of the given `owner/name` repos into $HOME, three at a time so
 * a big selection doesn't stampede the box. Returns the job map immediately;
 * the UI polls cloneStatus() for progress.
 */
export function startClones(names) {
  const accepted = [];
  for (const name of names) {
    if (!REPO_NAME_RE.test(name)) throw new Error(`Not a repository name: ${name}`);
    if (cloneJobs.get(name)?.status === "cloning") continue;
    const state = destState(name);
    if (state === "cloned") {
      cloneJobs.set(name, { status: "done" });
      continue;
    }
    if (state === "occupied") {
      // Never clone over an existing non-repo folder of the same name.
      cloneJobs.set(name, { status: "failed", error: `~/${name.split("/")[1]} already exists` });
      continue;
    }
    cloneJobs.set(name, { status: "cloning" });
    accepted.push(name);
  }
  // One shared chain of batches keeps at most 3 clones in flight even when
  // startClones is called again while earlier jobs are still running.
  for (let i = 0; i < accepted.length; i += 3) {
    const batch = accepted.slice(i, i + 3);
    cloneQueue = cloneQueue.then(() => Promise.all(batch.map(async (name) => {
      try {
        await execFileP("gh", ["repo", "clone", name, cloneDest(name)],
          { timeout: 10 * 60 * 1000, maxBuffer: 4 * 1024 * 1024 });
        cloneJobs.set(name, { status: "done" });
        console.log(`[github-repos] cloned ${name} -> ${cloneDest(name)}`);
      } catch (err) {
        const detail = (err.stderr || err.message || "").trim().split("\n").pop();
        cloneJobs.set(name, { status: "failed", error: detail });
        console.log(`[github-repos] clone of ${name} failed: ${detail}`);
      }
    })));
  }
  return cloneStatus();
