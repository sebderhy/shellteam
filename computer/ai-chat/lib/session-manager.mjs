...
export function interruptAgent(slotId = 0) {
  ...
  if (slot) slot.pendingAskUser = null;
}

/**
 * Answer a pending AskUserQuestion via the agent's control protocol.
 * Returns false when the slot's agent doesn't support structured answers or
 * the request is stale — the caller falls back to a normal message.
 */
export function answerQuestion(slotId, requestId, answers) {
  const slot = getSlot(slotId);
  if (!slot?.agent || typeof slot.agent.answerQuestion !== "function") {
    console.warn(`[session-mgr] Slot ${slotId}: no agent able to answer question ${requestId}`);
    return false;
  }
  const ok = slot.agent.answerQuestion(requestId, answers);
  if (ok) slot.pendingAskUser = null;
  return ok;
}
...
export async function sendMessage(slotId, content, { isWatchdogResend = false } = {}) {
  ...
  // A normal message while a question waits = the user answered in prose; the
  // Claude adapter denies the blocked tool call before writing the message.
  slot.pendingAskUser = null;
  touchSlot(slotId);
...
export function replayStateToSocket(ws, shouldIncludeSlot = () => true) {
  for (const [id, slot] of slots) {
    ...
    // A question still waiting on the user replays as a LIVE block (the
    // history copy renders with disabled buttons), so a reconnecting client
    // can still answer it.
    if (slot.isGenerating && slot.pendingAskUser) {
      ws.send(JSON.stringify({ type: "ask_user", ...slot.pendingAskUser, slot: id }));
    }
  }
}
...
function wireAgentEvents(slotId, agent) {
  ...
  const recordAuthFailure = (detail) => {
    const family = agentIdFor(slot.config.model);
    if (recordSubscriptionAuthFailure(family, detail)) {
      // The server's credential poll follows with a full status snapshot. This
      // event makes the cockpit warning immediate for the failed turn.
      broadcast({ type: "subscription_expired", family });
    }
  };
  ...
  on("ask_user", (data) => {
    touchSlot(slotId);
    // requestId present = answerable via the control protocol (Claude); the
    // pending copy lets reconnecting clients get a live, clickable block.
    slot.pendingAskUser = data.requestId ? data : null;
    addToHistory(slotId, { type: "ask_user", id: data.id, requestId: data.requestId, questions: data.questions });
  ...
  on("turn_done", (data) => {
    ...
    if (data.is_error) recordAuthFailure(data.errors);
  ...
  on("error", (data) => {
    recordAuthFailure(data?.message);
