...
export function loadModel() {
  // Resolve through the catalog: the persisted default may name a model that
...
const SUBSCRIPTION_FAMILIES = new Set(["claude", "codex", "antigravity"]);
...
function jwtExpiryMs(token) {
  if (!nonEmpty(token) || token.split(".").length < 2) return null;
  try {
    const payload = JSON.parse(Buffer.from(token.split(".")[1], "base64url").toString("utf8"));
    return expiryMs(payload.exp);
  } catch {
    return null;
  }
}

function tokenUsable(token, explicitExpiry = null) {
  if (!nonEmpty(token)) return false;
  const expiry = expiryMs(explicitExpiry) ?? jwtExpiryMs(token);
  return expiry === null || expiry > Date.now() + EXPIRY_SKEW_MS;
}

function classifyClaudeSubscription() {
  const oauth = readJsonSafe(CREDENTIALS_FILE)?.claudeAiOauth;
  if (!oauth || typeof oauth !== "object") return "none";
  const access = tokenUsable(oauth.accessToken, oauth.expiresAt);
  const refresh = tokenUsable(oauth.refreshToken, oauth.refreshTokenExpiresAt);
  if (access || refresh) return "connected";
  const evidence = nonEmpty(oauth.subscriptionType)
    || (Array.isArray(oauth.scopes) && oauth.scopes.length > 0)
    || nonEmpty(oauth.accessToken)
    || nonEmpty(oauth.refreshToken)
    || (expiryMs(oauth.expiresAt) ?? 0) > 0
    || (expiryMs(oauth.refreshTokenExpiresAt) ?? 0) > 0;
  return evidence ? "expired" : "none";
}

function classifyCodexSubscription() {
  const auth = readJsonSafe(CODEX_AUTH_FILE);
  if (!auth || typeof auth !== "object") return "none";
  const tokens = auth.tokens && typeof auth.tokens === "object" ? auth.tokens : {};
  if (tokenUsable(tokens.refresh_token) || tokenUsable(tokens.access_token)) return "connected";
  const evidence = String(auth.auth_mode || "").toLowerCase() === "chatgpt"
...
function classifyAntigravitySubscription() {
  const auth = readJsonSafe(ANTIGRAVITY_AUTH_FILE);
  if (!auth || typeof auth !== "object") return "none";
  const token = auth.token && typeof auth.token === "object" ? auth.token : {};
  if (tokenUsable(token.refresh_token) || tokenUsable(token.access_token, token.expiry)) return "connected";
  const evidence = nonEmpty(auth.auth_method)
    || nonEmpty(auth.id_token)
    || nonEmpty(token.access_token)
    || nonEmpty(token.refresh_token)
    || (expiryMs(token.expiry) ?? 0) > 0;
  return evidence ? "expired" : "none";
}

function staticSubscriptionStatus(family) {
  switch (family) {
    case "claude": return classifyClaudeSubscription();
    case "codex": return classifyCodexSubscription();
    case "antigravity": return classifyAntigravitySubscription();
    default: return "none";
  }
}

function credentialPathFor(family) {
  return {
    claude: CREDENTIALS_FILE,
    codex: CODEX_AUTH_FILE,
    antigravity: ANTIGRAVITY_AUTH_FILE,
  }[family] || null;
}

function credentialFingerprint(family) {
  const path = credentialPathFor(family);
  if (!path || !existsSync(path)) return null;
  try {
    return createHash("sha256").update(readFileSync(path)).digest("hex");
  } catch (err) {
    // Provider CLIs own these files and may replace one between the existence
    // check and read. Treat that instant as unavailable; the poll retries.
    console.warn(`[ai-chat] Could not fingerprint ${path}: ${err.message}`);
    return null;
  }
}

function readSubscriptionRecovery() {
  return readJsonSafe(SUBSCRIPTION_RECOVERY_FILE) || { version: 1, providers: {} };
}

function writeSubscriptionRecovery(state) {
  mkdirSync(dirname(SUBSCRIPTION_RECOVERY_FILE), { recursive: true });
  writeFileSync(SUBSCRIPTION_RECOVERY_FILE, JSON.stringify(state, null, 2));
  chmodSync(SUBSCRIPTION_RECOVERY_FILE, 0o600);
}

export function clearSubscriptionExpired(family) {
  if (!SUBSCRIPTION_FAMILIES.has(family)) return;
  const state = readSubscriptionRecovery();
  if (!state.providers?.[family]) return;
  delete state.providers[family];
  writeSubscriptionRecovery(state);
  console.log(`[ai-chat] Cleared ${family} subscription recovery warning`);
}

export function subscriptionStatusFor(family) {
  if (!SUBSCRIPTION_FAMILIES.has(family)) return null;
  const classified = staticSubscriptionStatus(family);
  const state = readSubscriptionRecovery();
  const marker = state.providers?.[family];
  if (!marker) return classified;

  const fingerprint = credentialFingerprint(family);
  if (fingerprint && marker.credentialFingerprint === fingerprint) return "expired";

  // A successful manual CLI login can rewrite credentials outside ShellTeam.
  // Retire the old failure marker as soon as the credential material changes.
  if (fingerprint) clearSubscriptionExpired(family);
  return classified;
}

export function markSubscriptionExpired(family, reason = "provider_rejected_oauth") {
  if (!SUBSCRIPTION_FAMILIES.has(family)) return false;
  const fingerprint = credentialFingerprint(family);
  if (!fingerprint || staticSubscriptionStatus(family) === "none") return false;
  const state = readSubscriptionRecovery();
  state.version = 1;
  state.providers ||= {};
  const previous = state.providers[family];
  if (previous?.credentialFingerprint === fingerprint) return false;
  state.providers[family] = {
    credentialFingerprint: fingerprint,
    reason,
    detectedAt: new Date().toISOString(),
  };
  writeSubscriptionRecovery(state);
  console.warn(`[ai-chat] ${family} subscription credentials were rejected; reconnect is required`);
  return true;
}

const SUBSCRIPTION_AUTH_ERRORS = {
  claude: [
    /oauth token.*(?:expired|invalid|revoked)/i,
    /invalid bearer token/i,
    /authentication_error.*(?:oauth|token)/i,
    /please (?:run \/login|log in again|sign in again)/i,
  ],
  codex: [
    /refresh token (?:has expired|was already used|has been revoked)/i,
    /access token.*(?:expired|invalid|revoked)/i,
    /please log out and sign in again/i,
    /not logged in.*(?:chatgpt|openai|codex)/i,
  ],
  antigravity: [
    /(?:oauth|refresh|access) token.*(?:expired|invalid|revoked)/i,
    /not authenticated.*(?:google|antigravity)/i,
    /(?:google|antigravity).*(?:sign in|log in) again/i,
  ],
};
...
export function authModeFor(family) {
  switch (family) {
    ...
    case "antigravity":
      ...
      return subscriptionStatusFor("antigravity") === "connected" ? "subscription" : "none";
    case "opencode":
...
export function hasOAuthCredentials() {
  return subscriptionStatusFor("claude") === "connected";
}


// --- Codex OAuth (device code flow) ---
export function hasCodexOAuthCredentials() {
  return subscriptionStatusFor("codex") === "connected";
}
...
export function hasAntigravityOAuthCredentials() {
  return subscriptionStatusFor("antigravity") === "connected";
}
...
export async function completeAntigravityAuth(code) {
  ...
  if (ok) {
    ...
    clearSubscriptionExpired("antigravity");
