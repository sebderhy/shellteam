...
// limits entirely) will answer the same way five minutes from now — respawning
// its CLI on every cache refresh (and every fresh page load) is pure churn.
// Its cached entry is reused for an hour; a forced refresh (the user opening
// the quota panel) always re-probes everything.
const UNAVAILABLE_REUSE_MS = 60 * 60_000;
...
/**
 * Which cached providers should be carried forward instead of re-probed:
 * those that answered without quota data ("unavailable") less than an hour
 * ago. Exported for tests.
 */
export function reusableProviders(cached, now = Date.now()) {
  const reuse = {};
  for (const [key, provider] of Object.entries(cached?.providers || {})) {
    const checkedAt = Date.parse(provider.checked_at || cached.generated_at || "") || 0;
    if (provider.status === "unavailable" && now - checkedAt < UNAVAILABLE_REUSE_MS) {
      reuse[key] = provider;
    }
  }
  return reuse;
...
export async function collectUsage(reuse = {}) {
  const result = {
  ...
  // Carry forward still-valid "no quota data" results — but only while the
  // auth mode is unchanged, so a fresh login always gets a real probe.
  const reused = new Set();
  for (const [key, cached] of Object.entries(reuse)) {
    if (result.providers[key] && cached.billing === result.providers[key].billing) {
      result.providers[key] = { ...cached };
      reused.add(key);
      console.log(`[usage] Reusing cached ${key} result (no quota data reported; skipping re-probe)`);
    }
  }
  ...
  // Stamp probe time per provider; reused entries keep their original stamp so
  // the hourly re-probe window is measured from the last REAL check.
  for (const [key, provider] of Object.entries(result.providers)) {
    if (!reused.has(key)) provider.checked_at = result.generated_at;
  }
  return result;
}
...
export async function collectUsageCached({ force = false } = {}) {
  ...
  usageInFlight = collectUsage(force ? {} : reusableProviders(cachedUsage))
    .then((usage) => {
      cachedUsage = usage;
