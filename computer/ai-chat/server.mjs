...
import {
  ...
  handleSpeak as sharedHandleSpeak,
  readBody as sharedReadBody,
  jsonResponse as sharedJsonResponse,
} from "../shared/media-handlers.mjs";
import { internalCallerAuthorized } from "./lib/internal-auth.mjs";
...
function authFlags() {
  ...
  return {
    ...
    sttAvailable: ai.stt,
    // Same key, the other direction: spoken replies. A guest cockpit holding
    // only a scoped STT token reads false here, which is the truth — voice
    // output is not lent with voice input.
    ttsAvailable: ai.tts,
    ...
    subscriptionStatus: {
      claude: subscriptionStatusFor("claude"),
      codex: subscriptionStatusFor("codex"),
      antigravity: subscriptionStatusFor("antigravity"),
    },
  };
}
...
// Provider CLIs can refresh or invalidate OAuth files while running, outside
// any ShellTeam endpoint. Watch the compact credential state and push changes
// to every cockpit without requiring a service restart or page reload.
let lastCredentialSnapshot = JSON.stringify(authFlags());
const credentialPoll = setInterval(() => {
  const flags = authFlags();
  const snapshot = JSON.stringify(flags);
  if (snapshot === lastCredentialSnapshot) return;
  lastCredentialSnapshot = snapshot;
  console.warn(`[ai-chat] Credential state changed: modes=${JSON.stringify(flags.authMode)} subscriptions=${JSON.stringify(flags.subscriptionStatus)}`);
...
const handleSpeak = sharedHandleSpeak;
...
const server = createServer((req, res) => {
  ...
  if (req.url === "/internal/dream/prune-stale-tabs" && req.method === "POST") {
    if (!internalCallerAuthorized(req, { label: "dream prune" })) {
      jsonResponse(res, 403, { error: "internal endpoint" });
      return;
    }
    readBody(req).then((body) => {
  ...
  // Environment Mirror: the control plane just received a laptop tarball and
  // wants a migration running before the user even opens the cockpit. This
  // STARTS AN AGENT from a caller-supplied prompt, so it is gated on the shared
  // internal secret + an Origin refusal (internalCallerAuthorized) — loopback
  // alone is not a boundary when the attacker is a page in the owner's browser.
  if (req.url === "/internal/mirror/kickoff" && req.method === "POST") {
    if (!internalCallerAuthorized(req, { label: "mirror kickoff" })) {
      jsonResponse(res, 403, { error: "internal endpoint" });
      return;
    }
    readBody(req).then(async (body) => {
      const { prompt, title } = JSON.parse(body.toString());
      if (!prompt) {
        jsonResponse(res, 400, { error: "prompt required" });
        return;
      }
      const slot = allocateSlotId(undefined, (id) => delegationBroker.isBrokerSlot(id));
      createSlot(slot, {});
      if (title) renameSlot(slot, title);
      addUserMessage(slot, prompt);
      await startAgent(slot);
      await sendMessage(slot, prompt);
      broadcast(buildStatus());
      console.log(`[ai-chat] Mirror kickoff: started migration in tab ${slot}`);
      jsonResponse(res, 200, { slot });
    }).catch((err) => jsonResponse(res, 400, { error: err.message }));
    return;
  }
      ...
      if (provider === "claude") {
        ...
        proc.on("close", (code) => {
          ...
          if (code === 0 && stdout.trim()) {
            ...
            if (recordSubscriptionAuthFailure("claude", errMsg)) broadcast(buildStatus());
        ...
        proc.on("close", (code) => {
          ...
          if (code === 0) {
            ...
            const errMsg = stderr.trim().split("\n").pop() || stdout.trim() || `Exit code ${code}`;
            if (recordSubscriptionAuthFailure("codex", errMsg)) broadcast(buildStatus());
            jsonResponse(res, 200, { success: false, error: errMsg });
          }
        });
        proc.on("error", (err) => {
        ...
        proc.on("close", (code) => {
          ...
          if (code === 0 && stdout.trim()) {
            ...
            if (recordSubscriptionAuthFailure("antigravity", errMsg)) broadcast(buildStatus());
            jsonResponse(res, 200, { success: false, error: errMsg });
          }
        });
        proc.on("error", (err) => {
  ...
  // Repo picker: list the signed-in user's repos, clone the selected ones
  // into ~/projects (used by the GitHub card in onboarding and Settings).
  if (req.url === "/api/github/repos" && req.method === "GET") {
    listGitHubRepos()
      .then((repos) => jsonResponse(res, 200, { repos }))
      .catch((err) => { console.log(`[github-repos] list failed: ${err.message}`); jsonResponse(res, 502, { error: err.stderr?.trim() || err.message }); });
    return;
  }

  if (req.url === "/api/github/clone" && req.method === "POST") {
    readBody(req).then((body) => {
      const { repos } = JSON.parse(body.toString());
      if (!Array.isArray(repos) || !repos.length) throw new Error("No repositories selected");
      jsonResponse(res, 200, { jobs: startGitHubClones(repos) });
    }).catch((err) => jsonResponse(res, 400, { error: err.message }));
    return;
  }

  if (req.url === "/api/github/clone-status" && req.method === "GET") {
    jsonResponse(res, 200, { jobs: gitHubCloneStatus() });
    return;
  }
...
chatWSS.on("connection", (ws, req) => {
  ...
  ws.on("message", async (data) => {
    ...
    switch (msg.type) {
      ...
      // A clicked option in an ask-user block. Answers the blocked
      // AskUserQuestion tool call via the control protocol when it's still
      // pending; otherwise (turn ended, agent restarted, replayed history) the
      // choice is delivered as a normal message so a click is never lost.
      case "answer_question": {
        const slot = msg.slot ?? 0;
        if (!requireSlot(slot, "answer_question", { content: msg.display })) break;
        const display = typeof msg.display === "string" && msg.display.trim() ? msg.display : "(answered)";
        addUserMessage(slot, display);
        if (!answerQuestion(slot, msg.requestId, msg.answers || {})) {
          console.log(`[ai-chat] answer_question for slot ${slot} fell back to a normal send (request ${msg.requestId} not pending)`);
          if (!isQueryActive(slot)) await startAgent(slot);
          await sendMessage(slot, display);
        }
        break;
      }
