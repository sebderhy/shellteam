...
const STALE_OPS = [
  ...
  ["setSlotModel", (sm, id) => sm.setSlotModel(id, "claude-opus-5")],
  ["switchSlotModel", (sm, id) => sm.switchSlotModel(id, "claude-opus-5")],
...
for (const [name, op] of STALE_OPS) {
  test(`SHE-52: stale ${name} on a closed slot never recreates it`, async () => {
    ...
    sm.createSlot(910001, { model: "claude-opus-5" });
...
test("SHE-52: two raced creates with the same hint get DISTINCT ids", async () => {
  ...
  sm.createSlot(a, { model: "claude-opus-5" });
  const b = sm.allocateSlotId(1);
  sm.createSlot(b, { model: "claude-opus-5" });
