...
test("SHE-89/91: a working tab is a tinted pill so the dot binds to its own tab", () => {
  // A bare leading dot between two adjacent tabs read as trailing the LEFT tab
  // when it belonged to the right one. The whole working tab must carry a
  // background tint so the pulsing dot is unmistakably inside its own pill.
  const rule = STYLES.match(/\.session-tab\.generating\s*\{([\s\S]*?)\}/);
  assert.ok(rule, ".session-tab.generating rule exists");
  assert.ok(
    /background:\s*var\(--accent-bg\)/.test(rule[1]),
    `working tab must be tinted, got: ${rule[1].trim()}`,
  );
});
