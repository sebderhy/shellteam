...
test("SHE-54: changing the workspace updates every surface at once", () => {
  ...
  // The header element can no longer hold uncommitted text at all: it is a
  // display-only button into the one modal picker (SHE-100 superseded the
  // old free-text combo whose blur handler had to undo stale typing).
  assert.match(appSrc, /function setWorkspaceDisplay\(path\)[\s\S]*?textContent = shortPath\(path\)/,
    "the header workspace display must be text set by setWorkspaceDisplay");
});
