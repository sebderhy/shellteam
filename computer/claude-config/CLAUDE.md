...
<!-- BEGIN:public -->
- Meant to go on the public web (portfolio, demo, blog) → `~/public/`, then
  publish it (see below). Putting a file there does NOT make it public on its
  own — publishing is a deliberate, owner-authorized step.
<!-- END:public -->
- Private project → `~/myproject/` (top-level in the home folder, where the
  Workspace picker lists it and cloned repos land)
...
By default, ports are private (owner-only). To let someone use an app, prefer an **expiring share link** — it grants that one port and lapses on its own:
...
Returns `{"url": ...}` to hand out. For endpoints that must accept anonymous traffic indefinitely (webhooks, public APIs), make the port fully public instead:
...
- `~/public/anything` → Owner only UNTIL published. Writing a file here does not
  expose it. To put something on the open web, the owner publishes it (the
  dashboard's publish toggle, or `POST /api/computers/reports` with the master
  token — which agents do not hold). You can build and stage freely in
  `~/public/`; the world-readable step is the owner's.
...
**Conversation archives (Claude Code sessions only):**
- When a long Claude Code conversation compacts, its full transcript is automatically saved to `~/conversations/` as markdown (a Claude Code PreCompact hook does this; Codex, Antigravity and OpenCode sessions are not archived)
