...
```bash
gh auth status 2>/dev/null && echo "GitHub: ready" || echo "GitHub: not connected (point user to the dashboard Apps tab → GitHub)"
...
> **GitHub note:** GitHub is **not** connected via Composio — the managed OAuth scopes are too narrow for typical dev workflows. Instead, the dashboard's "Connect GitHub" button runs `gh auth login --web` device-flow inside this container. If `gh auth status` shows logged out, point the user at the dashboard's Apps tab (GitHub card). Once connected, `gh` and `git push/pull` work for all coding agents.
...
- **GitHub:** `gh` CLI + `git push/pull` — user clicks "Connect GitHub" in the dashboard Apps tab (device-flow OAuth via the official `gh` CLI app). Composio is **not** used for GitHub.
