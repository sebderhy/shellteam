...
export function internalAiAuthHeaders() {
  let token = process.env.SHELLTEAM_AI_TOKEN;
...
/**
 * POST /speak {text} -> audio/mpeg. The client sends the agent's reply as it
 * was written; the markdown is stripped to speakable prose HERE so every
 * caller gets the same treatment and no client can spend synthesis quota on a
 * page of YAML.
 *
 * 204 (not an error) when a reply has nothing sayable in it — all code, table
 * or link. The cockpit shows no player and the text stands on its own.
 */
export async function handleSpeak(req, res) {
  ...
  try {
    ...
    jsonResponse(res, 400, { error: `Invalid request body: ${err.message}` });
    return;
  }

  const { text, truncated } = speakable(payload?.text);
  if (!text) {
    console.log("[media] TTS skipped: reply had no speakable prose");
    res.writeHead(204).end();
    return;
  }
  if (truncated) {
    console.log(`[media] TTS trimmed a long reply to ${text.length} chars for speech`);
  }
  ...
  try {
    const resp = await fetch(`${INTERNAL_API_BASE}/internal/ai/tts`, {
      method: "POST",
    ...
    if (!resp.ok) {
      ...
      console.error("[media] TTS failed:", resp.status, body.slice(0, 200));
    ...
    res.writeHead(200, {
      ...
      // The player is rendered next to the note that the reply was shortened.
      "X-Speech-Truncated": truncated ? "1" : "0",
    });
    res.end(audio);
  } catch (err) {
    console.error("[media] TTS error:", err.message);
    jsonResponse(res, 502, { error: "Speech service unavailable" });
  }
}
