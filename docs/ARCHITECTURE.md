...
| `shellteam-nginx` | `127.0.0.1:80` | File server: `~/` (owner-gated), published paths (world; publishing is the only grant — `~/public` is not ambient), `/_editor` Monaco SPA |
| `shellteam-steel` (Docker, opt-in) | `127.0.0.1:3877` (`STEEL_PORT`, off `:3000` for user apps) | Steel browser + bundled Chromium — the only Docker dependency; enabled by `./install.sh --full` (or `MODULES=browser` in `.env`) |
