...
This manifest is also **executable**: `./install.sh --plan` (plus any flags you
intend to use, e.g. `--plan --full --remote`) prints every change that exact
combination would make, evaluated against your box, and exits without touching
anything.
...
These touch the box outside ShellTeam's namespace. `uninstall.sh` reverses the two
that are purely ours (the nginx mask and the user-manager drop-in); the rest are
standard packages left in place.
...
| **Writes `/etc/systemd/system/user@<uid>.service.d/shellteam-restart.conf`** (`Restart=always`) | linger keeps your user manager alive but never restarts it if it *exits*, and a broadcast signal from any of your processes (`kill -1`, a broad `pkill`) makes it exit, taking every ShellTeam unit down until your next login | `uninstall.sh` removes it |
