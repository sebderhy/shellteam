...
#   ./install.sh --plan [flags]  # DRY RUN: print every change that combination
#                                #   of flags would make to this box (packages,
#                                #   services, files, network fetches) and exit
#                                #   without touching anything. The executable
#                                #   version of docs/FOOTPRINT.md.
#
...
KEEP_SUDO=0               # --keep-sudo: leave the --create-owner user with passwordless sudo after install
EXPLICIT_LOCAL=0          # --minimal / --no-start = a deliberate "just the core, no questions" choice
PLAN_ONLY=0               # --plan: print every mutation this flag combination would make, then exit
...
# ── Pinned third-party installers ─────────────────────────────────────────────
# Every script ShellTeam executes from the network is downloaded to a file and
# verified against a pinned sha256 BEFORE it runs — a changed (or compromised)
# upstream fails loudly instead of executing; the NodeSource one runs as root.
# When an upstream legitimately ships a new script the mismatch message prints
# the new hash: review the script, update the constant here, or re-run with
# SHELLTEAM_ALLOW_UNPINNED_INSTALLERS=1 to accept it once, with a warning.
#
# Pin VERSIONED artifacts wherever upstream publishes them. A "latest" URL and a
# sha256 are contradictory: the hash goes stale the moment upstream releases, and
# every fresh install then dies on a mismatch that is not a compromise. uv shipped
# 0.12.1 on 2026-08-03 and broke every install this way. A versioned URL is
# immutable, so the pin stays true until we deliberately bump it.
#
# Defined here, above print_plan, so `--plan` quotes the URLs the install really
# fetches instead of a copy that can drift.
NODESOURCE_SETUP_URL="https://deb.nodesource.com/setup_22.x"
...
UV_INSTALL_SHA256=d3f5412d38c99f9d024901843bf98206f0d2c6dbe64df40d0b740e2751ca62c1
...
# ── --plan: the executable version of docs/FOOTPRINT.md ───────────────────────
# Prints every change the given flag combination would make, evaluated against
# THIS box (read-only checks only), then exits before anything is touched.
print_plan() {
    local modules="${REQUESTED_MODULES:-none (pure core)}"
    echo "ShellTeam install plan — nothing has been changed. (Full manifest: docs/FOOTPRINT.md)"
    echo
    if [ -n "$CREATE_OWNER" ]; then
        echo "OWNER BOOTSTRAP (--create-owner $CREATE_OWNER, as root):"
        echo "  + create user '$CREATE_OWNER' (adduser --disabled-password)"
        if [ "$KEEP_SUDO" = 1 ]; then
            echo "  + write /etc/sudoers.d/$CREATE_OWNER — '$CREATE_OWNER ALL=(ALL) NOPASSWD:ALL' (kept: --keep-sudo)"
        else
            echo "  + write /etc/sudoers.d/$CREATE_OWNER for the install run only — removed when it finishes"
            echo "    (agents then hold only this user's files; pass --keep-sudo to keep root access)"
        fi
        echo "  + loginctl enable-linger $CREATE_OWNER"
        echo "    (docker group: only added by the browser module, which needs the Docker socket)"
        echo "  + copy this repo to /home/$CREATE_OWNER and re-run the installer as them"
        echo
    fi
    echo "SYSTEM PACKAGES (apt, as root):"
    echo "  + curl ca-certificates git nginx libcap2-bin python3 python3-venv build-essential sqlite3"
    if ! command -v node >/dev/null 2>&1 || [ "$(node -v 2>/dev/null | cut -dv -f2 | cut -d. -f1)" -lt 20 ]; then
        echo "  + nodejs 22.x (NodeSource apt repo; setup script sha256-pinned, see below)"
    else
        echo "  = node $(node -v 2>/dev/null) already present — kept as is"
    fi
    echo "  + gh (GitHub CLI; distro repo, else GitHub's signed apt repo) — non-fatal if unavailable"
    if [ "$PUBLIC_MODE" != none ]; then
        echo "  + caddy (Caddy's signed apt repo) — TLS reverse proxy for the public URL"
    fi
    if systemctl is-active --quiet nginx.service 2>/dev/null \
       || systemctl is-enabled --quiet nginx.service 2>/dev/null; then
        echo "  = an nginx service is active/enabled on this box — it is left UNTOUCHED"
        echo "    (ShellTeam runs its own systemd --user nginx on a free port next to it)"
    else
        echo "  + mask the system nginx.service (no nginx runs here today; apt's post-install"
        echo "    hook would otherwise auto-start it on :80). uninstall.sh unmasks it."
    fi
    echo
    echo "NETWORK-FETCHED INSTALLERS (downloaded to a file, sha256-verified against a"
    echo "pinned hash BEFORE running — a changed upstream aborts the install):"
    echo "  + $NODESOURCE_SETUP_URL (root) — only if Node <20"
    echo "  + $UV_INSTALL_URL (user) -> ~/.local/bin/uv"
    echo "  + $ANTIGRAVITY_INSTALL_URL (user) -> ~/.local/bin/agy"
    echo
    echo "GLOBAL npm CLIs (only those not already installed; existing ones are never touched):"
    echo "  + claude (@anthropic-ai/claude-code), codex (@openai/codex), opencode (opencode-ai),"
    echo "    context7-mcp (@upstash/context7-mcp)"
    echo
    echo "THIS USER'S SYSTEMD (--user units; removed by uninstall.sh):"
    echo "  + ~/.config/systemd/user/shellteam-{api,ai-chat,nginx}.service"
    echo "  + ~/.config/systemd/user/shellteam-update.{service,timer} (inert unless AUTO_UPDATE=1)"
    case ",$REQUESTED_MODULES," in *,dreaming,*)
        echo "  + ~/.config/systemd/user/shellteam-dream.{service,timer} (dreaming module)" ;;
    esac
    echo "  + loginctl enable-linger $(id -un) (services survive logout; reversed by uninstall.sh --purge)"
    echo "  + write /etc/systemd/system/user@$(id -u).service.d/shellteam-restart.conf (Restart=always; reversed by uninstall.sh)"
    echo
    echo "FILES WRITTEN (ShellTeam's own namespace only):"
    echo "  + $REPO/.env (from .env.example; auto-generates OWNER_TOKEN + SHELLTEAM_AI_TOKEN)"
    echo "  + ~/.local/state/shellteam/ (rendered nginx config, runtime state)"
    echo "  + ~/.shellteam/ (agent launch-layer, knowledge, session lineage)"
    echo "  modules: $modules"
    if [ "$PUBLIC_MODE" != none ]; then
        echo "  + /etc/caddy/Caddyfile (root) — TLS for ${PUBLIC_DOMAIN:-<dashed-ip>.sslip.io}"
    fi
    echo
    echo "PORTS (localhost binds; auto-picks the next free port if a default is taken):"
    echo "  api :8000, cockpit :3456, file server :80 (or :8080+ when :80 is contested;"
    echo "  setcap on OUR nginx binary only if the chosen port is <1024)"
    case ",$REQUESTED_MODULES," in *,browser,*)
        echo "  browser module: Steel container on 127.0.0.1:3877 (STEEL_PORT — off :3000"
        echo "    deliberately, so your own dev servers can use it)" ;;
    esac
    echo
    echo "NEVER TOUCHED — the additive guarantee (pinned by tests/test_purity_gate.py):"
    echo "  your dotfiles and coding-agent config: ~/.bashrc, ~/.gitconfig, ~/.claude*,"
    echo "  ~/.codex, ~/.gemini, ~/.config/opencode. Third-party installers that try to"
    echo "  edit shell profiles are snapshotted and reverted, loudly."
    echo
    echo "Run the same flags without --plan to install. Removal: ./uninstall.sh (--purge for state)."
}
...
bootstrap_owner() {
    ...
    # The install run needs root (apt, Caddy, the systemd drop-in). Grant it for
    # THIS run and take it back at the end (below): the owner is the user every
    # agent runs as, so leaving passwordless sudo behind makes any agent action
    # root. On a shared box that is the whole blast radius; on a dedicated box
    # it buys nothing. --keep-sudo keeps the old behaviour deliberately.
    if [ "${KEEP_SUDO:-0}" = 1 ]; then
        log "Granting '$user' passwordless sudo (/etc/sudoers.d/$user, kept: --keep-sudo)…"
    else
        log "Granting '$user' passwordless sudo for the install run (/etc/sudoers.d/$user, removed afterwards)…"
    fi
    ...
    # A login shell also WIPES the environment, so anything the operator set for
    # this run has to be forwarded explicitly or it silently vanishes here. The
    # unpinned-installer escape hatch is printed by our own error message, and it
    # was a no-op on this path (the only path that provisions a customer box)
    # until it was forwarded: the re-exec dropped it and the install failed again
    # with the identical mismatch. Keep this list in sync with any operator-facing
    # env var install.sh honours.
    local env_fwd=""
    if [ "${SHELLTEAM_ALLOW_UNPINNED_INSTALLERS:-0}" = 1 ]; then
        env_fwd+="SHELLTEAM_ALLOW_UNPINNED_INSTALLERS=1 "
    fi
    # The re-exec'd run learns it was bootstrapped, so its closing note can say
    # how to re-run later (as root again) once its sudo is gone.
    env_fwd+="SHELLTEAM_OWNER_BOOTSTRAP=$(printf '%q' "$user") "
    local rc=0
    # `exec env VAR=… ./install.sh`: a bare `exec VAR=… cmd` is a bash error
    # ("exec: VAR=…: not found"), which silently broke every forwarded variable
    # until fresh-box QA ran this path with one set.
    su - "$user" -c "cd $(printf '%q' "$dest") && exec env ${env_fwd}./install.sh $fwd" || rc=$?
    if [ "${KEEP_SUDO:-0}" != 1 ]; then
        rm -f "/etc/sudoers.d/$user"
        log "Removed the install-time sudo of '$user' (/etc/sudoers.d/$user). Agents on this box run as '$user' with no root access."
        log "  To re-run the installer later (add --full, switch to --remote…): sudo ./install.sh --create-owner $user <flags>"
    fi
    exit "$rc"
}
if [ -n "$CREATE_OWNER" ]; then
...
if [ "$(id -u)" -eq 0 ]; then
   ...
   it makes the account (sudo for the install run only, then removed; linger on)
   and re-runs itself as them. Any other flags you pass are forwarded to that run:
...
# The whole stack lives under this user's systemd manager (user@UID.service).
# Linger keeps that manager alive with no login session, but if the manager
# EXITS on its own nothing restarts it: only a fresh login brings it back.
# And exiting is exactly what a broadcast signal makes it do: a stray `kill -1`
# or a broad `pkill` from ANY process of this user (an agent cleaning up after
# itself, say) reaches the manager, which treats SIGINT/SIGTERM as exit.target,
# so every ShellTeam unit stops and the box stays dark until a human logs in
# (a 3.5h outage on 2026-09-07). A system-level drop-in makes systemd restart
# the manager unconditionally instead — the one root-owned file we write.
# See docs/decisions/20260907-user-manager-restart-always.md.
harden_user_manager() {
    local dropin
    dropin="/etc/systemd/system/user@$(id -u).service.d/shellteam-restart.conf"
    if sudo mkdir -p "$(dirname "$dropin")" 2>/dev/null \
        && printf '%s\n' \
            '# Written by ShellTeam install.sh; removed by uninstall.sh.' \
            '# ShellTeam runs under this user manager. If a broadcast signal (kill -1,' \
            '# a broad pkill) makes the manager exit, linger does NOT restart it and' \
            '# every service stays down until the next login. Restart it, always.' \
            '[Unit]' \
            'StartLimitIntervalSec=0' \
            '[Service]' \
            'Restart=always' \
            'RestartSec=2' \
            | sudo tee "$dropin" >/dev/null 2>&1 \
        && sudo systemctl daemon-reload 2>/dev/null; then
        return 0
    fi
    warn "Could not write $dropin — if your user manager ever exits, services stay down until you log in."
}
...
fetch_pinned_installer() {  # <url> <pinned-sha256> — prints the downloaded path
    ...
    f="$(mktemp /tmp/shellteam-installer.XXXXXX)"
    curl -fsSL "$url" -o "$f" || { rm -f "$f"; die "Download failed: $url"; }
    actual="$(sha256sum "$f" | cut -d' ' -f1)"
    if [ "$actual" != "$pinned" ]; then
        if [ "${SHELLTEAM_ALLOW_UNPINNED_INSTALLERS:-0}" = 1 ]; then
            warn "UNPINNED installer accepted (SHELLTEAM_ALLOW_UNPINNED_INSTALLERS=1): $url sha256 $actual (pinned: $pinned)"
        else
            rm -f "$f"
            die "Installer script changed upstream: $url
   pinned sha256: $pinned
   actual sha256: $actual
 Upstream may have shipped a legitimate update — or something worse. Review the
 script at that URL; if it looks right, update the pinned hash in install.sh
 (or re-run with SHELLTEAM_ALLOW_UNPINNED_INSTALLERS=1 to accept it once)."
        fi
    fi
    printf '%s' "$f"
}
...
if ! command -v node >/dev/null 2>&1 || [ "$(node -v | cut -dv -f2 | cut -d. -f1)" -lt 20 ]; then
    ...
    NODESOURCE_SCRIPT="$(fetch_pinned_installer "$NODESOURCE_SETUP_URL" "$NODESOURCE_SETUP_SHA256")"
...
if ! command -v uv >/dev/null 2>&1 && [ ! -x "$HOME/.local/bin/uv" ]; then
    ...
    UV_SCRIPT="$(fetch_pinned_installer "$UV_INSTALL_URL" "$UV_INSTALL_SHA256")"
    run_preserving_dotfiles "The uv installer" \
        env UV_NO_MODIFY_PATH=1 sh "$UV_SCRIPT"
    rm -f "$UV_SCRIPT"
fi
...
install_agy() {
    ...
    elif AGY_SCRIPT="$(fetch_pinned_installer "$ANTIGRAVITY_INSTALL_URL" "$ANTIGRAVITY_INSTALL_SHA256")" \
        && run_preserving_dotfiles "The Antigravity installer" \
            bash -c "bash '$AGY_SCRIPT' >/tmp/shellteam-agy-install.log 2>&1; rc=\$?; rm -f '$AGY_SCRIPT'; exit \$rc"; then
...
# Brand fonts the editor links as `fonts/fonts.css` — self-hosted, so the page
# loads nothing from a third party (and works with no outbound network).
install -d -m 0755 "$STATE/file-editor/fonts"
install -m 0644 "$REPO/frontend/static/vendor/fonts/"* "$STATE/file-editor/fonts/"
...
# on 127.0.0.1:$STEEL_PORT (the subdomain proxy forwards <user>-$STEEL_PORT there).
# The host port deliberately defaults OFF :3000 — that is the default dev port of
# Next.js/CRA/Rails, and the user's own `npm run dev` must be able to claim it
# (Steel hardcoded to :3000 made every such app die with EADDRINUSE on a --full
# box). The core stack is Docker-free, but Steel ships as a turnkey image bundling
# its own pinned Chromium — building it natively is impractical — so the optional
# browser runs as a loopback-only container. Idempotent: re-creates the container
# each run (an upgraded box migrates off :3000 on its next install run).
...
# Docker-socket access is root-equivalent, so nobody is put in the docker group
# just for existing — only the browser module needs it, and it asks for it here,
# where enabling the module IS the consent. For this run `sudo docker` covers the
# gap (the group lands on the next login).
dk() {
    if id -nG | grep -qw docker; then docker "$@"; else sudo docker "$@"; fi
}
ensure_docker_group() {
    id -nG | grep -qw docker && return
    warn "Adding $(id -un) to the docker group for the browser module — note that Docker-socket access is root-equivalent (see SECURITY.md)."
    sudo usermod -aG docker "$(id -un)" \
        || warn "could not add $(id -un) to the docker group — 'docker' commands need: sudo usermod -aG docker $(id -un)"
}
provision_browser() {
    ...
    # socket access) must never abort an install whose core stack is already
    # up. warn + skip, never die.
    if ! dk pull -q "$STEEL_IMAGE" >/dev/null; then
        warn "Failed to pull $STEEL_IMAGE (is the Docker daemon running?)."
        warn "  Browser tab skipped — rest of the stack is unaffected. Fix Docker and re-run, or remove 'browser' from MODULES in .env."
        return
    fi
    # Remove the old container FIRST so an autopick never flees our own port
    # (an idempotent re-run must land on the same STEEL_PORT, not walk away).
    dk rm -f "$STEEL_CONTAINER" >/dev/null 2>&1 || true
    local prev_steel_port
    prev_steel_port="$(env_val STEEL_PORT)"; prev_steel_port="${prev_steel_port:-3877}"
    STEEL_PORT="$prev_steel_port"
    while port_in_use "$STEEL_PORT"; do
        warn "Port :$STEEL_PORT is already in use — trying :$((STEEL_PORT + 1)) for the Steel browser."
        STEEL_PORT=$((STEEL_PORT + 1))
    done
    set_env STEEL_PORT "$STEEL_PORT"
    # The API booted (§11) reading the PREVIOUS value — converge it if autopick moved us.
    if [ "$STEEL_PORT" != "$prev_steel_port" ]; then
        systemctl --user restart shellteam-api || warn "failed to restart shellteam-api for STEEL_PORT=$STEEL_PORT"
    fi
    log "Starting Steel browser container (loopback 127.0.0.1:$STEEL_PORT)…"
    if ! dk run -d --name "$STEEL_CONTAINER" --restart unless-stopped \
        --shm-size=1g -p "127.0.0.1:$STEEL_PORT:3000" "$STEEL_IMAGE" >/dev/null; then
    ...
    for _ in $(seq 1 45); do
        if curl -fsS --max-time 3 "http://127.0.0.1:$STEEL_PORT/v1/health" >/dev/null 2>&1; then
            response="$(curl -fsS --max-time 30 -X POST \
                -H 'Content-Type: application/json' -d '{}' \
                "http://127.0.0.1:$STEEL_PORT/v1/sessions" 2>/dev/null || true)"
            session_id="$(printf '%s' "$response" | python3 -c \
                'import json,sys; print(json.load(sys.stdin).get("id", ""))' 2>/dev/null || true)"
            if [ -n "$session_id" ]; then
                curl -fsS --max-time 10 -X POST "http://127.0.0.1:$STEEL_PORT/v1/sessions/release" \
                    -H 'Content-Type: application/json' -d '{}' >/dev/null 2>&1 || true
                log "  Steel is up on :$STEEL_PORT (Chromium session launch verified)"
                return
            fi
        fi
        sleep 1
    ...
    dk logs --tail 30 "$STEEL_CONTAINER" >&2 || true
...
if [ -n "${SHELLTEAM_OWNER_BOOTSTRAP:-}" ] && [ "$KEEP_SUDO" != 1 ]; then
cat <<EOF
  No root for agents:   '${SHELLTEAM_OWNER_BOOTSTRAP}' loses its install-time sudo when this run ends.
                        Re-run the installer later (e.g. --full, --remote) from a root shell:
                          sudo ./install.sh --create-owner ${SHELLTEAM_OWNER_BOOTSTRAP} <flags>
EOF
fi
