...
const PAGES = [
  ...
  // The Reports tab shell (SHE-84): the search box is its one always-there
  // primary control; list items are data-dependent so geometry alone is
  // asserted for them.
  { path: "/reports", requiredActions: true, actions: ["#search"] },
  // The Apps tab shell: the search box is its always-there primary control;
  // app cards are catalog/connection-dependent so geometry alone is asserted.
  { path: "/apps", requiredActions: true, actions: ["#search"] },
  { path: "/", actions: [] },
...
for (const page of PAGES) {
  ...
  for (const [width, height] of page.viewports || WIDTHS.map((w) => [w, 780])) {
    // Emulation races navigation in headless Chrome: now and then the new
    // document lays out at the default 980 px instead of the requested width,
    // which reads as a bogus "overflow" or "hidden action". A measurement is
    // only valid when the document actually got the viewport it was asked for,
    // so re-apply and reload until it does (bounded), and fail loudly as a
    // HARNESS error, not a geometry verdict, if it never does.
    let emulated = false;
    for (let attempt = 0; attempt < 3 && !emulated; attempt++) {
      ...
      emulated = (await evaluate("innerWidth")) === width;
      if (!emulated) console.log(`     ${name} @${width}: viewport emulation did not apply (innerWidth=${await evaluate("innerWidth")}), retrying`);
    }
    if (!emulated) {
      console.error(`HARNESS ${name} @${width}×${height}: could not emulate the viewport after 3 attempts`);
      failures++;
      continue;
    }
    if (page.setup) {
