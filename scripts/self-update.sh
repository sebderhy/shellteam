...
# The release lineage can be restarted upstream (the public history collapsed
# to a fresh root). Following that is safe exactly when this checkout sits AT a
# release tag: there is nothing local on it to lose. Anything else (local
# commits, a dev branch) keeps the refusal.
RESTARTED=0
if ! git merge-base --is-ancestor "$FROM" "$TARGET"; then
    AT_TAG="$(git describe --tags --exact-match HEAD 2>/dev/null || true)"
    case "$AT_TAG" in
        v*) RESTARTED=1
            log "checkout is at release $AT_TAG, which is not an ancestor of $TAG: the release lineage was restarted upstream; following it." ;;
...
if [ "$RESTARTED" = 1 ]; then
    # Clean tree at a tag (checked above): moving the branch or the detached
    # HEAD onto the new root discards nothing.
    git reset --hard --quiet "$TARGET" || fail "reset to $TAG failed"
elif [ -n "$BRANCH" ]; then
