...
def test_skills_requiring_missing_binaries_are_excluded(home: Path):
    """The gws-* skills declare `metadata.openclaw.requires.bins: ["gws"]`, but
    no CLI honors that frontmatter at runtime (verified against Claude Code
    v2.1.215: `openclaw` is absent from the bundle), and native installs never
    provision the `gws` binary (`grep -c gws install.sh` is 0). Shipping the
    skills anyway taught agents commands that cannot run, so the builder must
    gate them on the binary's presence."""
    assert shutil.which("gws") is None, "test needs a gws-less PATH"
    build_agent_layer(home, "alice", modules=FULL)
    skills = {p.name for p in harness_skills_dir(home).iterdir()}
    assert not any(name.startswith("gws-") for name in skills), skills
    # Skills without unmet bin requirements still ship.
    assert {"stt", "external-apps", "frontend-design"} <= skills


def test_skills_with_required_binary_present_are_included(home: Path, monkeypatch):
    """Same skills, but with `gws` on PATH: the requirement is met, so the
    skills ship (the decision doc 20260712 keeps the gws-* family in scope; the
    gate is about the binary, not the skills)."""
    ...
    build_agent_layer(home, "alice", modules=FULL)
    skills = {p.name for p in harness_skills_dir(home).iterdir()}
    assert "gws-gmail" in skills and "gws-shared" in skills


def test_memory_prompt_scopes_compaction_archives_to_claude_code(home: Path):
    """The PreCompact transcript-archive hook ships ONLY in the Claude Code
    plugin (hooks are not delivered to Codex, Antigravity or OpenCode), so the
    shared prompt must not promise automatic ~/conversations/ archives
    unconditionally: for three of the four CLIs that claim was false."""
    build_agent_layer(home, "alice", modules=FULL)
    prompt = claude_system_prompt_file(home).read_text()
    assert (
        "When long conversations compact, the full transcript is automatically saved"
        not in prompt
    )
    assert "Conversation archives (Claude Code sessions only)" in prompt
    assert "Codex, Antigravity and OpenCode sessions are not archived" in prompt
