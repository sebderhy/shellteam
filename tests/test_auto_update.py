...
class TestSelfUpdateScript:
    ...
    def test_follows_a_restarted_release_lineage_when_at_a_tag(self, box, tmp_path):
        # Upstream collapsed its history: main is a fresh root, old tags gone,
        # one new tag. A box sitting exactly at a release tag follows it.
        origin = tmp_path / "origin"
        _git(origin, "checkout", "-q", "--orphan", "fresh")
        (origin / "VERSION").write_text("0.3.0\n")
        _git(origin, "add", "."); _git(origin, "commit", "-qm", "v0.3.0 (fresh root)")
        _git(origin, "branch", "-M", "main")
        _git(origin, "tag", "-d", "v0.1.0"); _git(origin, "tag", "-d", "v0.2.0")
        _git(origin, "tag", "v0.3.0")
        result, state = _run_update(box, tmp_path)
        assert result.returncode == 0, result.stderr
        assert (box / "VERSION").read_text() == "0.3.0\n"
        assert _git(box, "rev-parse", "HEAD") == _git(origin, "rev-parse", "v0.3.0")
        assert _git(box, "symbolic-ref", "--short", "HEAD") == "main"
        assert state["status"] == "ok" and "v0.3.0" in state["detail"]
        assert "restarted" in result.stdout

    def test_restarted_lineage_still_refuses_a_checkout_off_any_tag(self, box, tmp_path):
        # Same upstream collapse, but the box carries a local commit on top of
        # its release: not at a tag, so the refusal stands and nothing is lost.
        origin = tmp_path / "origin"
        _git(origin, "checkout", "-q", "--orphan", "fresh")
        (origin / "VERSION").write_text("0.3.0\n")
        _git(origin, "add", "."); _git(origin, "commit", "-qm", "v0.3.0 (fresh root)")
