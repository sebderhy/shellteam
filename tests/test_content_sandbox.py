...
class TestSandboxExemptionsAreFirstParty:
    ...
    def test_no_exemption_prefix_covers_content_dirs(self):
        ...
        from api.main import _TRUSTED_FILE_UI_PREFIXES, _wants_dashboard_csp

        content_roots = ("/reports", "/public", "/tmp", "/projects")
        for root in content_roots:
            for ex in _TRUSTED_FILE_UI_PREFIXES:
                assert not root.startswith(ex.rstrip("/")), (
                    f"exemption {ex!r} covers content dir {root!r}"
                )
            # _CSP_PATHS entries are exact-match (see _wants_dashboard_csp), so
            # a shell route may legitimately share its name with a content dir
            # (the /reports tab shell vs ~/reports files) — what must hold is
            # that the FILES under every content root still fall through to the
            # content sandbox, never the dashboard CSP.
            assert _wants_dashboard_csp(_req(f"{root}/x.html")) is False
            assert _wants_content_sandbox(_req(f"{root}/x.html"), _resp()) is True
...
# --- /reports is a first-party shell, never sandboxed content ----------------------

class TestReportsIsFirstPartyShell:
    """SHE-84 live-QA catch (2026-08-29): frontend/reports.html shipped exactly
    like /knowledge once did — missing from _CSP_PATHS, it fell through to
    CONTENT_SANDBOX_CSP, its top-level localStorage access threw under the
    opaque origin, and the tab rendered an empty list while every suite stayed
    green. Same lesson, pinned for the same reason."""

    def test_reports_page_gets_dashboard_csp_not_sandbox(self):
        with TestClient(app) as client:
            resp = client.get("/reports", headers={**LOCAL, **BEARER})
        assert resp.status_code == 200
        csp = resp.headers.get("content-security-policy", "")
        assert "default-src" in csp, f"/reports lacks the dashboard CSP: {csp!r}"
        assert "sandbox" not in csp, (
            "/reports is sandboxed — its opaque origin kills localStorage and "
            "turns the listing fetch into an Origin:null CORS failure"
        )

    def test_reports_is_explicitly_in_csp_paths(self):
        from api.main import _CSP_PATHS, _wants_dashboard_csp

        assert "/reports" in _CSP_PATHS
        assert _wants_dashboard_csp(_req("/reports")) is True
        assert _wants_content_sandbox(_req("/reports"), _resp()) is False
        # The shell exemption is the literal path only — the report FILES one
        # directory down stay sandboxed (test_owner_html_stays_sandboxed_...).
        assert _wants_dashboard_csp(_req("/reports/x.html")) is False
