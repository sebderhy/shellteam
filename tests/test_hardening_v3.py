...
class TestContentSecurityPolicy:
    ...
    def test_csp_denies_google_fonts(self, client):
        """CSP must NOT allow Google Fonts — the brand fonts are self-hosted under
        /static/vendor/fonts/ and /vendor/fonts/, so no page load reaches a third
        party. Adding fonts.googleapis.com back = regression."""
        resp = client.get("/", headers={"host": "localhost"})
        csp = resp.headers.get("content-security-policy", "")
        assert "fonts.googleapis.com" not in csp, f"CSP must not allow Google Fonts: {csp}"
        assert "fonts.gstatic.com" not in csp, f"CSP must not allow Google Fonts: {csp}"
...
# ---------------------------------------------------------------------------
# 3. No shipped page may load a font (or anything else) from a third party
# ---------------------------------------------------------------------------
class TestNoThirdPartyFonts:
    """Brand fonts are self-hosted under frontend/static/vendor/fonts/ and
    computer/ai-chat/public/vendor/fonts/.

    A single `<link href="https://fonts.googleapis.com/...">` silently reopens
    a third-party request on every page load, which contradicts the product's
    "nothing phones home" promise and breaks the cockpit on a box with no
    outbound network. The CSP blocks it now, so a reintroduced link would fail
    closed (unstyled text) rather than loudly — hence this test.
    """
    ...
    def _shipped_files():
        ...
        # docs/archive holds historical planning documents, not shipped UI.
        skip = ("node_modules", "docs/archive", ".git")
        for sub in ("frontend", "computer"):
    ...
    def test_no_page_references_a_third_party_font_host(self):
        offenders = [
            f"{rel} → {host}"
            for rel, path in self._shipped_files()
            for host in self.THIRD_PARTY_HOSTS
            if host in path.read_text(encoding="utf-8", errors="ignore")
        ]
        assert not offenders, "third-party font hosts must stay self-hosted: " + ", ".join(offenders)

    def test_brand_fonts_are_actually_present(self):
        """The self-hosted files must exist, or the pages above render in a
        fallback face and the whole point is lost."""
        import pathlib
        root = pathlib.Path(__file__).resolve().parent.parent
        for vendor in ("frontend/static/vendor/fonts",
                       "computer/ai-chat/public/vendor/fonts"):
            d = root / vendor
            assert (d / "fonts.css").is_file(), f"missing {vendor}/fonts.css"
            woff2 = list(d.glob("*.woff2"))
            assert len(woff2) >= 10, f"{vendor} has only {len(woff2)} woff2 files"
