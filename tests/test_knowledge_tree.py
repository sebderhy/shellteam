...
class TestWriteSemantics:
    ...
    def test_dream_review_all_queues_every_confidence(self, tmp_path, monkeypatch):
        """DREAM_REVIEW=all is the paranoid mode: even a high-confidence op
        waits for owner review before it can influence future agents
        (SECURITY.md 'Dreaming and memory poisoning')."""
        monkeypatch.setenv("DREAM_REVIEW", "all")
        result = kt.apply_delta(tmp_path, "acme-project", [op(confidence="high")], "r1")
        assert result.counts()["queued"] == 1
        assert not (kt.node_dir(tmp_path, "acme-project") / "index.md").exists()
        # Owner approval still lands the fact (force bypasses the queue only).
    ...
    def test_extract_prompts_frame_transcripts_as_attempted_injection_data(self):
        """Both extraction prompts must carry the anti-poisoning framing: text
        inside a transcript that asks to be remembered is an injection, not a
        fact. Regression pin for the dreaming persistence threat model."""
        from api.services import dreaming

        for prompt in (dreaming.EXTRACT_PROMPT, dreaming.OWNER_PROMPT):
            assert "attempted injection" in prompt
