...
class TestSessionCookiesNeverReachUpstreams:
    ...
    def test_ws_upstream_headers_strip_every_platform_credential(self):
        """The WS proxy forwards the app's OWN cookies + Origin upstream (so
        authenticated app sockets work) but must strip every ShellTeam platform
        credential — same guarantee as the HTTP sanitizer. Asserted through the
        real sanitizer, not by banning header pass-through outright."""
        from api.routers.proxy import _sanitized_ws_upstream_headers, _SESSION_COOKIES

        raw_cookie = "; ".join(
            [f"{c}=secret-{c}" for c in _SESSION_COOKIES] + ["app_session=keepme", "csrf=alsokeep"]
        )
        headers = {
            b"cookie": raw_cookie.encode(),
            b"origin": b"https://alice-5173.localhost",
            b"x-forwarded-for": b"192.0.2.4",
            b"x-forwarded-by": b"nginx",
        }
        out = _sanitized_ws_upstream_headers(headers)
        forwarded_cookie = out.get("Cookie", "")
        for c in _SESSION_COOKIES:
            assert c not in forwarded_cookie, f"{c} leaked to the upstream app socket"
        assert "app_session=keepme" in forwarded_cookie
        assert "csrf=alsokeep" in forwarded_cookie
        assert out.get("Origin") == "https://alice-5173.localhost"
        # No forwarding/trust headers ride along (M1).
...
class TestProxyForwarding:
    ...
    def _publish_public_paths(self):
        # These forward /public/ paths; ~/public is no longer ambient (F4),
        # so treat the path as published to exercise the forward path.
    ...
    async def test_port_forwarding(self):
        ...
        assert resp.status_code == 200
        assert b"dev server" in await _response_body(resp)
    ...
    async def test_does_not_record_port_hit_when_http_proxy_fails(self):
        ...
        with (
            ...
            # Patch `send`, not `request`: the app-port path streams via
            # build_request+send, so a `.request` patch misses it and the test
            # makes a REAL connection attempt (passed locally where the fake IP
            # refuses fast, 504'd on CI where it times out).
            patch(
...
class TestProxyAuthGate:
    ...
    @pytest.mark.asyncio
    @respx.mock
    async def test_published_path_skips_ownership_check(self):
        """A published path uses resolve_username_owner but skips the auth check."""
        respx.get("http://172.20.0.5:80/public/index.html").mock(
...
async def _response_body(resp):
    """Body bytes of a plain OR streaming response (app ports now stream)."""
...
class TestWebSocketProxyAuth:
    ...
    @pytest.mark.asyncio
    async def test_ws_forwards_negotiated_subprotocol(self):
        """The client's Sec-WebSocket-Protocol must reach the upstream and the
        upstream's selection must be echoed in the accept — Vite's HMR client
        requires the `vite-hmr` subprotocol and hangs on a bare accept."""
        port_service._public_ports["owner-1"] = {5173}
        scope = _ws_scope("alice-5173.localhost", path="/", forwarded_for="203.0.113.99")
        scope["headers"].append((b"sec-websocket-protocol", b"vite-hmr"))
        sent = []
        ...
        with (
            ...
            patch(
                "api.routers.proxy.websockets.connect", return_value=_ConnectCtx()
            ) as connect,
        ):
            await proxy_websocket(scope, receive, send)

        assert connect.call_args.kwargs.get("subprotocols") == ["vite-hmr"]
        accept = next(m for m in sent if m.get("type") == "websocket.accept")
        assert accept.get("subprotocol") == "vite-hmr"

    @pytest.mark.asyncio
    async def test_ws_forwards_app_cookie_strips_platform_cookie(self):
        """An authenticated app socket needs its OWN cookie upstream, but no
        ShellTeam credential may ride along. Files cookie authenticates the owner
        at the gate, then is stripped from the upstream connect; the app's own
        cookie survives."""
        from api.services.auth import files_token

        port_service._public_ports["owner-1"] = {4000}
        host = "alice-4000.localhost"
        scope = _ws_scope(
            host,
            path="/socket",
            cookies={"shellteam_files": files_token(), "app_sid": "keepme"},
        ...
        with (
            ...
            patch("api.routers.proxy.websockets.connect", return_value=_ConnectCtx()) as connect,
        ):
            await proxy_websocket(scope, receive, send)

        fwd = connect.call_args.kwargs.get("additional_headers") or {}
        cookie = fwd.get("Cookie", "")
        assert "app_sid=keepme" in cookie, "app's own socket cookie was dropped"
        assert "shellteam_files" not in cookie, "files credential leaked to the app socket"
...
class TestPublicPortBypass:
    ...
    async def test_public_port_skips_auth_http(self):
        ...
        with patch(
            ...
            resp = await proxy_subdomain(request, "")
        assert resp.status_code == 200
        assert b"public app" in await _response_body(resp)

    @pytest.mark.asyncio
    async def test_non_public_port_still_requires_auth(self):
        """Port NOT marked public → still needs cookie. (Port 8001, not 8000:
        API_PORT is reserved now and 404s before auth — covered in
        test_fullstack_apps.py.)"""
        port_service._public_ports["owner-1"] = {3000}
        request = _make_request(
            "alice-8001.localhost",
            cookies={},
            forwarded_for="203.0.113.99",
        )
        with patch(
...
class TestIsPublicPath:
    """~/public is no longer ambient (exposure-audit F4): a path is public only
    when it (or a parent dir) has been explicitly published. Needs owner_id."""

    def test_unpublished_public_folder_is_not_public(self):
        # A bare ~/public write no longer grants world-read.
        assert _is_public_path("public/index.html", 80, "owner") is False
        assert _is_public_path("public", 80, "owner") is False

    def test_published_file_is_public(self):
        with patch.object(reports, "is_report_public",
                          side_effect=lambda uid, p: p == "public/index.html"):
            assert _is_public_path("public/index.html", 80, "owner") is True
            assert _is_public_path("/public/index.html", 80, "owner") is True
            assert _is_public_path("public/other.html", 80, "owner") is False

    def test_published_directory_covers_children(self):
        with patch.object(reports, "is_report_public",
                          side_effect=lambda uid, p: p == "public/logos" or p.startswith("public/logos/")):
            assert _is_public_path("public/logos/a.svg", 80, "owner") is True

    def test_no_owner_is_not_public(self):
        assert _is_public_path("public/index.html", 80, None) is False

    def test_private_path(self):
        assert _is_public_path("projects/secret", 80, "owner") is False

    def test_non_file_port(self):
        with patch.object(reports, "is_report_public", return_value=True):
            assert _is_public_path("public/index.html", 3000, "owner") is False
...
class TestMainHostFileServing:
    ...
    # --- Public bypasses: explicitly published paths only (F4) ---

    @pytest.mark.asyncio
    @respx.mock
    async def test_unpublished_public_folder_now_needs_a_token(self):
        # ~/public is no longer ambient — an unpublished path 401s (F4).
    ...
    @pytest.mark.asyncio
    @respx.mock
    async def test_published_public_file_needs_no_token(self):
        respx.get(f"{self.FILE_ORIGIN}/public/share.html").mock(
            return_value=httpx.Response(200, text="shared")
        )
        with patch("api.routers.proxy.reports.is_report_public", return_value=True):
...
class TestShareFooter:
    ...
    @pytest.mark.asyncio
    @respx.mock
    async def test_published_public_folder_html_is_never_badged(self):
        # ~/public sites (portfolios, demos) stay unbadged even when published.
