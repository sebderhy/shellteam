...
USER_MANAGER_DROPIN="/etc/systemd/system/user@$(id -u).service.d/shellteam-restart.conf"
if [ -f "$USER_MANAGER_DROPIN" ]; then
    log "Removing the user-manager restart drop-in (install.sh wrote it)…"
    if sudo rm -f "$USER_MANAGER_DROPIN" 2>/dev/null; then
        sudo rmdir "$(dirname "$USER_MANAGER_DROPIN")" 2>/dev/null || true   # only when now empty
        sudo systemctl daemon-reload >/dev/null 2>&1 || true
    else
        warn "Could not remove $USER_MANAGER_DROPIN — remove it manually."
    fi
fi
